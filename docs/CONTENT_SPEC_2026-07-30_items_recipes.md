...
## 14. スレッド15種の提案（ビルド分岐用）

### 14-0. 前提（実装の裏取り 2026-07-30）

- スレッドのステは**実際に効く**。socket 済み防具スレッドの item-stats は `AddonCombatStats` が
  プレイヤー単位の正規ステマップへ合算し、`PlayerDefenseResolver` / `WeaponAttackStatResolver` が読む。
  PDC にキャッシュされ、**同種スレッドの個数によるセットボーナス**も別枠で乗る
- スロットは**防具のみ 5枠**（`crafting-features.yml` の `thread-slots.max-by-category` = `armor: 5 / weapon: 0 / tool: 0 / other: 0`）
- 使えるステは `stats/lore.yml` の **119キー**が語彙の正。以下の提案は全て実在キー
- 構成ルール（要望どおり）: **固定ステ1つ ＋ 残りはロール**。
  サブステ3・4（⭐︎）は**付与自体が確率**（`advanced.randomize-grants: true` ＋ `grant-chances`）
- 強さは 1〜5 の相対表記。**実数への変換は要判断**（§13-B）

| 構成要素 | item-stats での書き方 |
|---|---|
| 固定ステータス | `fixed:` に1キー（品質で伸ばすなら `per-quality:` も） |
| サブステ1・2 | `random:` にレンジ（必ず抽選される） |
| サブステ3・4（⭐︎） | `random:` ＋ `advanced.randomize-grants: true` / `grant-chances.<key>: 0.3〜0.5` |

### 14-1. ビルド網羅マップ

| 系統 | スレッド | 主なゲーム的役割 |
|---|---|---|
| 物理・会心バースト | 雷紋の一閃 | 単発火力の天井を上げる |
| 物理・対重装 | 鎧断ちの錐 | 防御率が高い敵への回答（貫通） |
| 物理・対守備力 | 秤の刃 | flat守備力が高い敵への回答（固定ダメージ） |
| 物理・継戦DoT | 蝕みの糸 | 攻撃速度と噛み合う出血 |
| 物理・範囲 | 群狼の呼び声 | 増援の多いダンジョン向け |
| 弓 | 遠雷の弦 | 距離を取る立ち回りを成立させる |
| 防御・反撃 | 静寂の反響 | タンクが火力に参加する手段 |
| 防御・回避 | 陽炎の残像 | 軽装ビルドの軸 |
| 防御・壁 | 不動の礎 | 重装ビルドの軸（固定ダメージに弱い＝対策が存在する） |
| 生存 | 命脈の縫合 | ソロ・長期戦（修繕廃止環境での価値） |
| 魔法・バースト | 星霜の詠唱 | 魔法の天井 |
| 魔法・持続 | 潮汐の環 | マナ制限を緩める |
| 魔法・拡張 | 古術士の遺稿 | 使えるグリフの幅を広げる |
| 生産・採取 | 実りの手 | ソース経済に直結（非戦闘ビルド） |
| 収集・厳選 | 賭博師の指先 | 図鑑/ガチャ/品質厳選に効く |
...
サブステ1:移動速度(3)
サブステ2:軽装セット回避(3)
サブステ3（ランダム）⭐︎:自然回復量(2)
サブステ4（ランダム）⭐︎:攻撃速度加算(2)

名前:古術士の遺稿
入手難度（レア度）:4
固定ステータス:Ars Tier追加(3)
サブステ1:グリフ枠追加(3)
サブステ2:最大マナ(3)
サブステ3（ランダム）⭐︎:ソース消費軽減(3)
サブステ4（ランダム）⭐︎:魔法耐性(2)
...
サブステ1:最大マナ(3)
サブステ2:待機マナ回復率(3)
サブステ3（ランダム）⭐︎:魔法守備力(2)
サブステ4（ランダム）⭐︎:マナ回復間隔(2)
...
サブステ1:採掘運(3)
サブステ2:スキル経験値増加(3)
サブステ3（ランダム）⭐︎:素材返還率(2)
サブステ4（ランダム）⭐︎:伐採追加ドロップ率(2)
...
サブステ1:ドロップ品質(3)
サブステ2:ガチャ確率UP(2)
サブステ3（ランダム）⭐︎:討伐ドロップ増加(3)
サブステ4（ランダム）⭐︎:釣り運(2)
```

### 14-3. 設計上の噛み合わせメモ

- **三すくみが崩れない**: 「鎧断ちの錐（貫通＝防御率だけを無視）」と「秤の刃（固定ダメージ＝flat装甲を相殺）」で
  敵の防御タイプごとに別解が要る形にしている。貫通は耐性・flat には効かない仕様なので、片方だけでは全対応できない
- **上限に張り付かせない**: 回避率は `damage.yml defense.max-dodge-chance: 0.9`、耐性/軽減は `max-mitigation-rate: 0.9` で
  クランプされる。「陽炎の残像」を積み過ぎても無敵にならない
- **セット効果と重ね掛け**: 同種スレッド個数のセットボーナスが別枠で乗るため、
  「不動の礎」「命脈の縫合」「実りの手」は `stackable: true` ＋ `max: 3` 程度が向く。
  逆に「雷紋の一閃」「星霜の詠唱」「静寂の反響」は `stackable: false`（1枠だけの尖った枠にする）
- **軽装セット回避**は `NativeAttributeBridge.SET_BONUS_MIN_PIECES = 3` で**3部位以上**が条件（説明文の「2部位以上」は誤り、K-5参照）
- **距離ダメージ**は16ブロック単位で加算・**64ブロックで頭打ち**。レベル5でも無限に伸びない
- **非戦闘スレッドを入れる意味**: 「実りの手」「賭博師の指先」は戦闘力ゼロだが、
  ソース経済（§2）と厳選（品質）に直結するので、**装備枠を戦闘以外に使う選択**が生まれる

### 14-4. ⚠ 保留が必要なキー（ACTIVE_RECORD W-4）

`hit-mana-recovery`（命中マナ回復）/ `damage-mana-recovery`（被弾マナ回復）/
`mana-cost-reduction-percent` / `mana-cost-reduction-flat` の4キーは、
**ArsPaper フォークの `TrinityForgeBridge` がメインハンドしか読まない**ため、
**防具スレッド由来の値は乗らない可能性が高い**（W-4 が未解消）。

このため上の15種では、魔法系の固定ステを `mana-regen`（時間マナ回復）/ `mana-bonus`（最大マナ）/
`glyph-damage-multiplier-bonus`（グリフダメ倍率）に寄せてある。
W-4 解消後に「命中マナ回復」を主軸にした魔法スレッドを追加するのが安全。

### 14-5. レア度と入手経路の対応（案）

| レア度 | 入手経路 | 該当 |
|---|---|---|
| 5 | 束縛者ダンジョンの宝箱／最終段階ボス | 雷紋の一閃・静寂の反響・星霜の詠唱 |
| 4 | 大型ダンジョンの宝箱（ガチャ `treasure` プールの最高レア枠） | 鎧断ちの錐・秤の刃・遠雷の弦・陽炎の残像・古術士の遺稿 |
| 3 | 反復ダンジョン宝箱／構造物チェスト（引換証経由） | 蝕みの糸・群狼の呼び声・不動の礎・潮汐の環 |
| 2 | 採取のドロップ表（釣り・掘削・伐採） | 命脈の縫合・実りの手・賭博師の指先 |

**クラフト可否**: 憲法「スレッドは特殊効果系のみクラフト・残りは厳選」に従い、
上記15種は**すべてクラフト不可（厳選専用）**とする。既存の `thread_empty` から作れる効果スレッド群がクラフト枠。

---

## 15. ランダム枠を4つへ拡張 ＋ 追加5種（計20種）

### 15-1. 構成の変更（⭐︎枠 2 → 4、確率を下げる）

| | 変更前 | 変更後 |
|---|---|---|
| 固定ステータス | 1 | 1（変更なし） |
| サブステ（常時ロール） | 2 | 2（変更なし） |
| **サブステ（⭐︎ ランダム）** | **2枠 × 付与率50%** | **4枠 × 付与率25%** |
| 付与される⭐︎の期待本数 | 1.0本 | **1.0本（同じ）** |

**期待値は同じで、分散だけが増える**のが狙い。当たり本数の分布はこうなる:

| 当たった⭐︎の本数 | 確率 | プレイヤーの体感 |
|---|---|---|
| 0本 | 31.6% | ハズレ（解体して再挑戦） |
| 1本 | 42.2% | 標準 |
| 2本 | 21.1% | 良品 |
...
- **厳選が「本数 × 品質」の二軸になる**。品質(0〜9)は各ステの値の大きさを決め、⭐︎の本数は付いた種類数を決める
  → 「4本乗ったけど品質3」「品質9だけど0本」のような**語れる個体差**が生まれる
- `grant-chances` は**ステキー単位**で設定できるので、**強いステだけ確率を下げる**運用にする:

| ⭐︎のステの強さ | 付与率 | 例 |
|---|---|---|
| 強（火力の天井を上げる） | **15%** | 固定ダメージ・グリフダメ倍率・会心ダメージ・追加ダメ |
| 中 | **25%** | 会心率・貫通率・出血率・守備力系・耐性系 |
| 軽（利便・非戦闘） | **35%** | 移動速度・リーチ・満腹度・採取系・EXP系 |

```yaml
# item-stats.yml の書き方（例: 雷紋の一閃）
  "SILENCE_ARMOR_TRIM_SMITHING_TEMPLATE#300020":
    fixed:
      crit-damage: 0.0            # ← 強さ5相当の実数（要判断）
    random:
      crit-chance:   { min: 0.0, max: 0.0 }   # サブ1（常時）
      attack-power:  { min: 0.0, max: 0.0 }   # サブ2（常時）
      fixed-damage:  { min: 0.0, max: 0.0 }   # ⭐︎3
      percent-bonus-damage: { min: 0.0, max: 0.0 }  # ⭐︎4
      attack-speed-bonus:   { min: 0.0, max: 0.0 }  # ⭐︎5
      stun-chance:          { min: 0.0, max: 0.0 }  # ⭐︎6
    advanced:
      randomize-grants: true
      grant-chances:
        crit-chance: 1.0          # 常時枠は 1.0 にする
        attack-power: 1.0
        fixed-damage: 0.15        # 強
        percent-bonus-damage: 0.15
        attack-speed-bonus: 0.25  # 中
        stun-chance: 0.25
```

> **重要**: `randomize-grants: true` は `fixed` / `per-quality` / `random` に書いた**全ステ**を確率化の対象にするため、
> **常時ロールさせたいサブステ1・2 は `grant-chances` に明示的に `1.0` を書く**（未記載キーは 1.0 だが、
> 意図を config 上に残すため書いておく）。固定ステも同様に 1.0 を明示する。

### 15-2. 既存15種に追加する⭐︎枠（⭐︎5・⭐︎6）

| スレッド | ⭐︎5 | ⭐︎6 |
|---|---|---|
| 雷紋の一閃 | 攻撃速度加算(2) | スタン率(2) |
| 静寂の反響 | 魔法守備力(3) | 体力増強(2) |
| 星霜の詠唱 | 時間マナ回復(3) | グリフ枠追加(2) |
| 鎧断ちの錐 | 攻撃速度加算(2) | リーチ(2) |
| 秤の刃 | 貫通率(2) | 防具強度(2) |
| 遠雷の弦 | 会心ダメージ(3) | 弓CT短縮(3) |
| 陽炎の残像 | リーチ(2) | 幸運(2) |
| 古術士の遺稿 | グリフダメ倍率(2) | 儀式品質(2) |
| 蝕みの糸 | 攻撃力(2) | 移動速度(2) |
| 群狼の呼び声 | 出血率(2) | アイテムCT短縮(2) |
| 不動の礎 | 反射率（実）(2) | 重装セットKB耐性(2) |
| 潮汐の環 | グリフダメ倍率(2) | 自然回復量(2) |
| 命脈の縫合 | 物理守備力(2) | 隠し満腹度回復量(2) |
| 実りの手 | 収穫追加ドロップ率(2) | 怪しいブロック復活(2) |
| 賭博師の指先 | 釣りボーナス(2) | 養蜂幸運(2) |
...
サブステ1:魔法守備力(4)
サブステ2:防具強度(3)
サブステ3（ランダム）⭐︎:被ダメ軽減(3)
サブステ4（ランダム）⭐︎:体力増強(3)
サブステ5（ランダム）⭐︎:反射率（実）(2)
サブステ6（ランダム）⭐︎:最大マナ(2)
...
サブステ1:空中攻撃半径(3)
サブステ2:追撃ノックバック(3)
サブステ3（ランダム）⭐︎:会心ダメージ(3)
サブステ4（ランダム）⭐︎:固定ダメージ(2)
サブステ5（ランダム）⭐︎:スタン率(3)
サブステ6（ランダム）⭐︎:K耐性(2)
...
サブステ1:儀式品質(4)
サブステ2:ロール上振れ(3)
サブステ3（ランダム）⭐︎:上振れ拡大(3)
サブステ4（ランダム）⭐︎:ソース消費軽減(3)
サブステ5（ランダム）⭐︎:材料節約率(2)
サブステ6（ランダム）⭐︎:ラピス消費軽減(2)
...
サブステ1:討伐時バニラEXP(3)
サブステ2:常時バニラEXP(2)
サブステ3（ランダム）⭐︎:破壊時バニラEXP(2)
サブステ4（ランダム）⭐︎:幸運(2)
サブステ5（ランダム）⭐︎:満腹度回復量(2)
サブステ6（ランダム）⭐︎:移動速度(2)
```
...
| スレッド | 埋めた穴 | 噛み合わせ |
|---|---|---|
| **静謐の帷** | 既存の防御スレッドが物理寄りだった。**対魔法の最上位**が無かった | 魔法偏重ダンジョン（`arcane_ward` テーマ）への回答。束縛者は逆に魔法防御が厚い＝こちらは他ダンジョン用。「不動の礎(物理)」との使い分けが生まれる |
| **天墜の楔** | `power-attack-damage` / `power-attack-radius`（空中攻撃）が誰も使っていなかった | 落下攻撃を主軸にする立ち回り。重量武器の遅さを「跳んで叩く」で別解にする |
| **名匠の見立て** | 制作コンテンツ（§6）に効く装備枠が無かった | `ritual-quality-bonus` / `workbench-quality-bonus` / 上振れ系はクラフト厳選の直接強化。ソース儀式が主産業なので `ソース消費軽減` が刺さる |
| **疾風の刻み** | 手数型（攻撃速度）が固定枠として無かった | 出血（命中トリガ）と乗算的に噛み合う。`アイテムCT短縮` が乗ると重量武器のCTも緩む |
| **学びの糸** | 育成加速（EXP）の枠が無かった | 全職レベルマ（サブゴール）と `combatレベル = 物理柱/魔法柱の max` の進行に直接効く。戦闘力ゼロだが伸びが速い＝**序盤〜中盤に強い選択** |

### 15-5. レア度と入手経路（20種の最終割り）

| レア度 | 入手 | 該当（計20種） |
|---|---|---|
| 5 | 束縛者ダンジョン | 雷紋の一閃・静寂の反響・星霜の詠唱・**静謐の帷** |
| 4 | 大型ダンジョン宝箱（ガチャ `treasure` の最高レア枠） | 鎧断ちの錐・秤の刃・遠雷の弦・陽炎の残像・古術士の遺稿・**天墜の楔**・**名匠の見立て** |
| 3 | 反復ダンジョン／構造物チェスト（引換証経由） | 蝕みの糸・群狼の呼び声・不動の礎・潮汐の環・**疾風の刻み** |
| 2 | 採取のドロップ表（釣り・掘削・伐採） | 命脈の縫合・実りの手・賭博師の指先・**学びの糸** |

CMD は `300020〜300039` に収まる（20種）。全種クラフト不可（厳選専用）。
