...
ここから出る、設計上いちばん重要な結論2つ:

1. **燃料1個あたりの価値を上げても生産速度は上がらない。** バッファは無制限に貯まり、排出だけが 50/5秒で
   律速する。価値の高い燃料は「**投入頻度を下げる＝自動化の手間を減らす**」効果しか持たない。
   （**石炭ブロック1個/5秒（価値45）で既にほぼ上限**。溶岩バケツなら1個/10秒で上限）
2. **カスタムソースリンクを増やしても1台の速度は変わらない。** `sourcelinks.yml` の `items.<id>` で追加できるのは
   「同じ type の挙動を持つ別の見た目の装置」で、値テーブルは type 共有・ドレイン上限は共通。
   → **現状スケールできる軸は「台数」だけ**。

```
必要実効稼働時間[h] = 100,000,000 ÷ (設備数 N × 36,000)
```
...
**結論: 現状の実装のまま1億を目標にすると「ソースリンクを何十台並べるゲーム」になる**（しかも常時チャンクロード必須）。
インクリメンタルとしては一応成立するが、強化の実感が「設置数」しか無く、サーバ負荷にも直結する。1億を掲げるなら:

- **(A) ドレイン上限を設備ティア別に config 化する**（ArsPaper 側の小改修・§7-A）。**推奨**。
  「ティアの高い炉は速い」という**倍率の実感**が作れて、台数を増やさずに済む。
  40〜50時間規模を狙うなら最終ティアで **D ≈ 4,000/5秒**（8台で約43時間）。目標時間は要判断（§9）
- **(B) 目標の単位を「累計投資ソース」に読み替える**（貯蔵量ではなく儀式で払った総額）。
  ドレイン上限を触らずに桁は作れるが、**"メーターが増える"実感は薄い**
- **(C) 1億という数字自体を下げる**（例: 100万ソース）。改修ゼロで成立する（8台で3.5時間、28台で1時間）

以下は (A) 前提で書く。(B) を採るなら §2-3 のコスト表をそのまま「累計投資額」の定義として使えばよい。
(C) を採るなら §2-3 の全コストを1/100にすればそのまま流用できる。

### 2-2. ループの構造（クッキークリッカーの"3ノブ"に対応させる）

| インクリメンタルの要素 | この設計での実体 | 触るファイル | 現状 |
|---|---|---|---|
| **生産速度**（本体） | ソースリンクのドレイン上限（ティア別） | （§7-A の改修が必要） | **1台10/秒で固定** |
| **生産台数** | 儀式で作る追加ソースリンク（`items.<id>` ＋ `type:`） | `sourcelinks.yml` | 編集可 |
| **投入の手間**（自動化QoL） | 燃料1個あたりの価値（Material 単位） | `sourcelinks.yml` の `volcanic/mycelial/alchemical.materials` | 編集可（**速度ではなく頻度に効く**） |
...
**世代（Gen）を8段にして、コストもドレイン上限も ×4 で揃える**。こうすると「1世代を上げるのに必要な時間が一定」になり、
体感が"詰まらない"。8世代の儀式コスト累計がちょうど 1億。
（**満杯ジャーは排出分が消滅する**ので、容量アップは"損失を止める強化"として体感が出る。詰まり解消のノブとして優秀）

### 2-3. 世代表（初期値提案）

**生産の主軸＝ドレイン上限**（§7-A の改修前提）。燃料価値とジャー容量は「手間と損失」を減らす副軸。

| 世代 | 生産（ドレイン上限 /5秒） | 副軸で解放されるもの | 追加要求素材 | 儀式コスト | 累計 |
|---|---|---|---|---|---|
| G1 | 50（現状） | 標準ジャー 10,000 | — | 4,600 | 4,600 |
| G2 | 100 | 燃料 45帯運用（石炭ブロック）／ジャー 40,000 | 鉄・銅 | 18,400 | 23,000 |
| G3 | 200 | 「ソース練炭」(価値 500) | ①フィールド高レベル素材 | 73,600 | 96,600 |
| G4 | 400 | ジャー 160,000 | 反復ダンジョン素材 | 294,000 | 390,600 |
| G5 | 800 | 「魔導練炭」(価値 2,000) | 反復ダンジョンボス素材 | 1,180,000 | 1,570,600 |
| G6 | 1,600 | ジャー 640,000 | 高レベル帯(Lv65+)限定素材 | 4,710,000 | 6,280,600 |
| G7 | 3,200 | 「次元炉心」(価値 8,000) | **束縛者ダンジョン素材** | 18,800,000 | 25,080,600 |
| G8 | 6,400 | 「世界樹の残滓」＋無限ジャー(`capacity: -1`) | **束縛者 最終段階素材** | 75,400,000 | **100,480,600** |

所要時間の目安（`N台 × D × 720 /時`）: G8 到達後は8台で **6,400×720×8 = 3,686万/時** 出るので、
最終世代のコスト 7,540万 は約2時間。累計1億のうち大半は G5〜G7 帯（D=800〜3,200）で溶ける計算になり、
**総プレイ時間はおおむね 40〜60 時間規模**。ここは目標時間の決定（§9-1）に合わせて ×4 の倍率を ×3 や ×5 に振って調整する。
...
### 7-A. ソースリンクのドレイン上限（①の前提・最重要）

- 現状: `Sourcelink.MAX_DRAIN_PER_TICK = 50` / `SourcelinkTickTask.TICK_INTERVAL = 100`（**どちらもハードコード定数**）
  → 1台10/秒で固定。**カスタムソースリンクを定義しても1台の速度は変えられない**（§2-1）
- 必要: **ティア別に config 化**（`sourcejars.yml` の `capacity` と同じ発想で、`sourcelinks.yml` の各 type / 各 `items.<id>` に
  `drain-per-tick` を持たせる。`items.<id>` 側にも書けると「上位炉」が作れて設計の幅が出る）
- 規模: 定数1つを config 読み出しに変える程度＋エディタのフォームに数値欄1つ
...
- **要確認**: 稼働中の ArsPaper フォーク（`fork-handoff/arspaper/fork/`、公開リポジトリには含まれない）は、
  ...
  ドレインも同様に対応済みの可能性があるため、**着手前に稼働フォークの実コードで確認**（P0）
...
1. **①の目標プレイ時間**。1億に何時間かけさせるか（40h / 100h / 400h）。これが §7-A の `drain-per-tick` 最終値を決める。
   **改修ゼロで行くなら目標を100万ソースへ下げる**（§2-1 案C）のも選択肢
